...
import { Sparkles, Plus, PanelLeftClose, PanelLeftOpen, Trash2, ChevronDown, FolderPlus, Pencil, MoreHorizontal } from 'lucide-react';
import { useUiStore } from '@/lib/store';
import { useChat } from '@/app/(main)/chat/_lib';
import { chatUrl } from '@/app/(main)/chat/_lib/chat-url';
...
export function ChatPageContent({ conversationId, folderId }: ChatPageContentProps) {
  const {
    conversations,
    folders,
  ...
  const toggleFolderMenu = (e: React.MouseEvent, menuFolderId: string) => {
    e.preventDefault();
    e.stopPropagation();
    setFolderMenuOpenId(prev => prev === menuFolderId ? null : menuFolderId);
    setConversationMenuOpenId(null);
  };

  const toggleConversationMenu = (e: React.MouseEvent, convId: string) => {
    e.preventDefault();
    e.stopPropagation();
    setConversationMenuOpenId(prev => prev === convId ? null : convId);
  ...
  const handleFolderDelete = (deletedFolderId: string, folderName: string) => {
    setDeleteConfirmation({
      isOpen: true,
      conversationId: deletedFolderId,
      conversationTitle: folderName,
      type: 'course',
    });
    setFolderMenuOpenId(null);
  };
  ...
  useEffect(() => {
    // Set folder breadcrumb
    if (folderId) {
      const folder = folders.find(f => f.id === folderId);
      if (folder) {
        setBreadcrumbOverride(folderId, folder.name);
      }
    }
    // Set conversation / view breadcrumb
    if (viewState === 'course-setup') {
      setBreadcrumbOverride('new-course', 'New Course Setup');
    } else if (currentConversationId) {
      const title =
        conversations.find(c => c.id === currentConversationId)?.title
        || folders.flatMap(f => f.conversations).find(c => c.id === currentConversationId)?.title;
      if (title) {
        setBreadcrumbOverride(currentConversationId, title);
      }
    }
  }, [currentConversationId, folderId, conversations, folders, setBreadcrumbOverride, viewState]);
  ...
  // Navigation: new standalone chat
  const handleNewChat = () => {
    startNewChat();
    closeViewer();
    if (folderId) {
      // Currently in a folder context — cross segment
      router.push('/chat');
    } else {
  ...
  const handleConversationClick = (targetConversationId: string, targetFolderId?: string) => {
    ...
    const targetUrl = chatUrl(targetConversationId, targetFolderId);
    const sameSegment = (folderId ?? null) === (targetFolderId ?? null);

    closeViewer();

    if (sameSegment) {
      // Same route segment — pushState for performance (no re-mount)
      loadMessages(targetConversationId);
      window.history.pushState(null, '', targetUrl);
    } else {
      // Different segment — let Next.js re-mount with correct folderId
      router.push(targetUrl);
    }
  };

  const handleDeleteClick = (targetConversationId: string, title: string) => {
    setDeleteConfirmation({
      isOpen: true,
    ...
    try {
      if (deleteConfirmation.type === 'course') {
        ...
        if (result.success) {
          ...
          // If we were viewing this folder, navigate to standalone chat
          if (folderId === deleteConfirmation.conversationId) {
            router.push('/chat');
          }
        } else {
  ...
  const handleCreateCourseComplete = (newFolderId: string) => {
    showSuccessToast('Course created successfully');
    // Always cross-segment: navigate to new folder's chat
    router.push(chatUrl(undefined, newFolderId));
  };

  const handleBatchUploadSuccess = (uploadConversationId: string) => {
    setIsBatchModalOpen(false);
    if (uploadConversationId !== currentConversationId) {
      handleConversationClick(uploadConversationId, folderId);
    }
  };
  ...
  return (
    <div className="absolute inset-0 flex w-full overflow-hidden">
                        ...
                        folder.conversations.map((conversation) => {
                          ...
                          return (
                            <div key={conversation.id} className={cn(
                              ...
                              <button
                                onClick={() => handleConversationClick(conversation.id, folder.id)}
                                disabled={isLoading}
                                className={cn(
                                  'w-full text-left px-3 py-2.5 rounded-xl text-sm transition-all duration-200 truncate',
                                  isActive
      ...
      <BatchUploadModal
        ...
        folderId={folderId}
        conversationId={folderId ? null : currentConversationId}
        folderContext={false}
      />
