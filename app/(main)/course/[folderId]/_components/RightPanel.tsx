...
// ──────────────────────────────────────────────────────
// RightPanel — Collapsible glass drawer with:
//   • "Activity" tab (conversation history)
//   • "Files" tab (knowledge base)
//   • "Inspector" mode (node detail on node click)
// ──────────────────────────────────────────────────────
        ...
        return (
            <div className="flex flex-col items-center justify-center p-8 text-center">
                <div className="w-12 h-12 rounded-xl bg-[color:var(--muted)]/50 flex items-center justify-center mb-3">
                    <MessageSquare className="w-6 h-6 text-[color:var(--muted-foreground)]" />
                </div>
    ...
    return (
        <div className="divide-y divide-[color:var(--border)]/20">
            {conversations.map((conv) => (
                ...
                >
                    <div
                        className="shrink-0 p-2 rounded-lg bg-[color:var(--muted)] text-[color:var(--muted-foreground)] group-hover:text-white transition-all"
                        style={{
                            "--tw-bg-opacity": "1",
                    ...
                    <ChevronRight className="w-3.5 h-3.5 text-[color:var(--muted-foreground)] opacity-0 group-hover:opacity-100 transition-opacity" />
                </button>
            ))}
        </div>
    );
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center p-8">
                ...
                <p className="text-xs text-[color:var(--muted-foreground)] mt-2">Loading files…</p>
            </div>
        );
    }
        ...
        return (
            <div className="flex flex-col items-center justify-center p-8 text-center">
                ...
                <button
                    ...
                    className="mt-3 px-4 py-2 text-xs font-medium rounded-lg text-white transition-all hover:opacity-90"
        ...
        <div className="p-2 space-y-1">
            {files.map((file) => {
                ...
                return (
                    <div
                        key={file.id}
                        className="group flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[color:var(--muted)]/40 transition-colors"
                    >
                        <div
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-[color:var(--foreground)] truncate" title={file.filename}>
                                {file.filename}
                            </p>
                            <div className="flex items-center gap-1.5 mt-0.5">
                                ...
                                <div className={cn("flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-medium", sc.bgColor)}>
                                    <StatusIcon className={cn("w-2.5 h-2.5", file.status === "processing" && "animate-spin")} style={{ color: sc.color }} />
                        ...
                        <button
                            ...
                            className="shrink-0 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 text-[color:var(--muted-foreground)] hover:text-red-500 hover:bg-red-500/10 transition-all"
                            title="Delete file"
                        >
    ...
    return (
        <div className="p-4 space-y-5">
            ...
            <div className="space-y-2">
                <h3 className="text-lg font-bold text-[color:var(--foreground)] leading-tight">
                    {label}
                </h3>
                {docType && (
            ...
            {description && (
                <div>
                    <h4 className="text-[10px] font-semibold uppercase tracking-wider text-[color:var(--muted-foreground)] mb-1">
                        Summary
                    </h4>
            ...
            {fileId && (
                <div className="space-y-2 pt-2 border-t border-[color:var(--border)]/30">
                    <button
                        onClick={() => router.push(`/chat/${fileId}`)}
                        className={cn(
                    ...
                    >
                        ...
                        <div className="flex-1">
                            <span className="text-sm font-medium text-[color:var(--foreground)]">Open Chat</span>
                            <span className="text-[10px] text-[color:var(--muted-foreground)] block">
                    ...
                    <button
                        onClick={() => window.open(`/chat/${fileId}`, "_blank")}
                        className={cn(
                            "w-full flex items-center gap-3 p-3 rounded-xl text-left",
                            "border border-[color:var(--border)]/30",
                            "hover:bg-[color:var(--muted)]/40 transition-all group"
                        )}
                    >
                        <div className="w-8 h-8 rounded-lg bg-[color:var(--muted)] flex items-center justify-center">
                            <ExternalLink className="w-4 h-4 text-[color:var(--muted-foreground)]" />
                        </div>
            ...
            {!fileId && (
                <div className="text-sm text-[color:var(--muted-foreground)] italic p-4 text-center rounded-xl bg-[color:var(--muted)]/20">
                    Structural node — no file linked
                </div>
            )}
        </div>
    );
